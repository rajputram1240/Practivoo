...
type FindTheMistakesProps = Omit<QuesAnsComponentsProps, "activeQuesType">;

const FindTheMistakes = ({
    current,
    activeIndex,
    updateCurrent,
    addOption,
    ...
    return (
        <div>
            ...
            <div>
                ...
                <textarea
                    placeholder="eg. rat is also know as mouse elephant (Find wrong word in sentence)"
                    className="w-full border p-2 rounded-lg"
                    value={current.question}
                    onChange={(e) => updateCurrent({ question: e.target.value })}
                />

                {/*   <div className='border flex p-2 rounded-lg mt-2'>
                    <div className='border p-2 gap-2 items-center rounded-lg flex'>
                        <input className="outline-0" placeholder='Type Here' />
                        <Trash2 size={18} className="text-black bg-amber-50 ml-2" />
                        <Check />
                    </div>

                </div> */}
                <p className="text-md ml-2 my-2">
            ...
            {/* Options */}
            <div>
                <label className="font-semibold">Add Options </label>
                <div className="space-y-2 mt-2 rounded-lg gap-2 flex border p-2">
                    {current.options.map((opt: string, idx: number) => (
                        ...
                        >
                            <input
                                value={opt}
                                placeholder="Tick the wrong one"
...
export default FindTheMistakes; 